...
let fbchurch_if_then = "(Fun c_bool -> Fun x -> Fun y -> c_bool ("^thaw "x"^") ("^thaw "y"^") )";;

(* Let omega = (Fun x -> x x) In
   Let cond = fbchurch_or church_true church_false In
   fbchurch_if_then cond (Fun _ -> 5) (Fun _ -> omega omega) ;; *)
